...
**This is the canonical workflow for all feature work in this repo.** It combines
`superpowers` skills (idea + execution discipline) with `spec-kit` skills (spec-driven
procedure). Spec-kit skills are installed under `.claude/skills/speckit-*`; superpowers
skills are installed at user scope and available everywhere.
...
plan, to de-risk ambiguity) → `/speckit-plan` → `/speckit-tasks`. The installed
`extensions.yml` auto-offers an agent-context refresh after specify and after plan — accept it.
...
- `superpowers:subagent-driven-development` — execute independent tasks via subagents (aligns
  with this repo's "use a subagent per phase" rule above).
